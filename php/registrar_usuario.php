...
try {
    ...
    $check = $conn->prepare("SELECT id FROM usuarios WHERE LOWER(email) = LOWER(?) LIMIT 1");
    if (!$check) {
        throw new Exception('No se pudo comprobar el email en la base de datos.');
    }
...
if ($hash === false) {
    ...
    $stmt = $conn->prepare("INSERT INTO usuarios (nombre, email, contrasena, celular) VALUES (?, ?, ?, ?)");
    if (!$stmt) {
        throw new Exception('No se pudo preparar el registro en la base de datos.');
    }
    $stmt->bind_param('ssss', $nombre, $email, $hash, $celular);
