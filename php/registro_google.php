...
$stmt = $conn->prepare("SELECT id, nombre, correo, rol, tipoCuenta, imagen_perfil FROM usuarios WHERE google_sub = ? OR correo = ? LIMIT 1");
...
if ($user) {
    ...
    echo json_encode(['success' => true, 'message' => 'Inicio de sesión correcto', 'user' => ['id'=>$_SESSION['user_id'],'nombre'=>$_SESSION['user_name'],'correo'=>$_SESSION['user_email'],'rol'=>$_SESSION['rol']]]);
    exit;
}
...
$_SESSION['rol'] = 'usuario'; // rol por defecto, igual que registrar_usuario.php

echo json_encode(['success' => true, 'message' => 'Usuario creado e iniciada sesión', 'user' => ['id'=>$newId,'nombre'=>$name,'correo'=>$email,'rol'=>$_SESSION['rol']]]);
exit;
?>