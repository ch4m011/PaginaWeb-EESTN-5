...
try {
    ...
    $stmt = $conn->prepare("SELECT id, nombre, email, contrasena, tipo, google_sub, rol FROM usuarios WHERE email = ?");
    $stmt->bind_param("s", $email);
    $stmt->execute();
    $res = $stmt->get_result();
    error_log('[google_login] Búsqueda en BD por email: ' . $email . '. Filas encontradas: ' . ($res ? $res->num_rows : 'NULL'));

    if ($res && $res->num_rows > 0) {
        $usuario = $res->fetch_assoc();
        error_log('[google_login] Usuario encontrado: ID=' . $usuario['id'] . ', tipo=' . $usuario['tipo'] . ', google_sub=' . ($usuario['google_sub'] ?? 'NULL'));

        // Caso: ya registrado como cuenta Google (google_sub coincide o tipo=google)
        if (!empty($usuario['google_sub']) && $usuario['google_sub'] === $google_sub) {
            ...
            // iniciar sesión
        } elseif ($usuario['tipo'] === 'google' && empty($usuario['google_sub'])) {
            ...
            error_log('[google_login] Caso 4: otros casos, actualizando google_sub y tipo');
            // Otros casos: actualizar google_sub y tipo
            $upd2 = $conn->prepare("UPDATE usuarios SET google_sub = ?, tipo = 'google' WHERE id = ?");
            $upd2->bind_param("si", $google_sub, $usuario['id']);
            if ($upd2->execute()) {
                error_log('[google_login] UPDATE google_sub y tipo exitoso');
            } else {
        ...
        error_log('[google_login] Sesión iniciada. user_id=' . $_SESSION['id'] . ', session_id=' . session_id());
    ...
    $stmtIns = $conn->prepare("INSERT INTO usuarios (nombre, email, tipo, google_sub, rol) VALUES (?, ?, 'google', ?, 'usuario')");
