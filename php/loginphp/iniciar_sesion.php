...
try {
    ...
    if ($user = $result->fetch_assoc()) {
        ...
        debug_log('Resultado password_verify: ' . (password_verify($password, $user['contrasena']) ? 'TRUE' : 'FALSE'));
        $hash = $user['contrasena'];

        debug_log('=== DEBUG LOGIN ===');
        debug_log('Password ingresada: ' . $password);
        debug_log('Hash BD: ' . ($hash ?? 'NULL'));
        debug_log('Hash length: ' . strlen($hash ?? ''));
        debug_log('password_verify: ' . (password_verify($password, $hash ?? '') ? 'TRUE' : 'FALSE'));
