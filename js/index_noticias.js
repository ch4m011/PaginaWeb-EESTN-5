...
function renderNoticiasGrid(noticias) {
    ...
    const heroImgSrc = (primary.imagen && String(primary.imagen).trim() !== '') ? (typeof normalizarRuta === 'function' && typeof getBasePath === 'function' ? (primary.imagen.startsWith('/') ? normalizarRuta(primary.imagen) : normalizarRuta(getBasePath() + primary.imagen)) : primary.imagen) : '../img/logo-tecnica.png';
    ...
    hero.innerHTML = `
        <a class="hero-link" href="../html/ver_noticia.html?id=${encodeURIComponent(primary.id)}">
            ${heroBadge}
            <img class="hero-image" src="${escapeHtml(heroImgSrc)}" alt="${escapeHtml(primary.titulo)}" onerror="this.src='../img/logo-tecnica.png'">
            ${heroCaption ? `<div class="hero-caption">${escapeHtml(heroCaption)}</div>` : ''}
        </a>
        <div class="news-body">
            <h2 class="news-title"><a href="../html/ver_noticia.html?id=${encodeURIComponent(primary.id)}">${escapeHtml(primary.titulo)}</a></h2>
            ...
            <div class="news-meta">
                ...
                <a class="news-readmore" href="../html/ver_noticia.html?id=${encodeURIComponent(primary.id)}">Leer más</a>
            </div>
        </div>
    `;
    ...
    secundarias.forEach((n, idx) => {
        ...
        const imgSrc = (n.imagen && String(n.imagen).trim() !== '') ? (typeof normalizarRuta === 'function' && typeof getBasePath === 'function' ? (n.imagen.startsWith('/') ? normalizarRuta(n.imagen) : normalizarRuta(getBasePath() + n.imagen)) : n.imagen) : '../img/logo-tecnica.png';
        ...
        const autorFoto = (n.imagen_perfil ? (typeof getBasePath === 'function' && typeof normalizarRuta === 'function' ? (n.imagen_perfil.startsWith('/') ? normalizarRuta(n.imagen_perfil) : normalizarRuta(getBasePath() + n.imagen_perfil)) : `../img/perfiles/${n.imagen_perfil}`) : '../img/logo-tecnica.png');

        card.innerHTML = `
            <a class="news-image-link" href="../html/ver_noticia.html?id=${encodeURIComponent(n.id)}">
                <img class="news-image ${imgClass}" src="${escapeHtml(imgSrc)}" alt="${escapeHtml(n.titulo)}" onerror="this.src='../img/logo-tecnica.png'">
            </a>
            <div class="news-body">
                ...
                <div class="news-meta">
                    <div style="display:flex; align-items:center; gap:8px;">
                        <img src="${escapeHtml(autorFoto)}" alt="${autor}" style="width:28px;height:28px;border-radius:50%;object-fit:cover;" onerror="this.src='../img/logo-tecnica.png'">
