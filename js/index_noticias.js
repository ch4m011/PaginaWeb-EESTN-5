...
function renderNoticiasGrid(noticias) {
    ...
    hero.innerHTML = `
        <a class="news-featured__link" href="../html/ver_noticia.html?id=${encodeURIComponent(primary.id)}">
            <div class="news-featured__media">
                <img class="news-featured__image" src="../php/imagen_noticia.php?id=${primary.id}" alt="${escapeHtml(primary.titulo)}" onerror="this.src='../img/logo-tecnica.png'">
    ...
    secundarias.forEach((n) => {
        ...
        const autorNombre = escapeHtml(n.autor_nombre || 'Anónimo');

        card.innerHTML = `
            <a class="news-image-link" href="../html/ver_noticia.html?id=${encodeURIComponent(n.id)}">
                <img class="news-image" src="../php/imagen_noticia.php?id=${n.id}" alt="${escapeHtml(n.titulo)}" onerror="this.src='../img/logo-tecnica.png'">
            </a>
            <div class="news-body">
                ...
                <div class="news-meta">
                    <div style="display:flex; align-items:center; gap:8px;">
                        <img src="../php/perfil/obtener_imagen.php?id=${n.autor_id}" alt="${autorNombre}" onerror="this.src='../img/logo-tecnica.png'" style="width:24px;height:24px;border-radius:50%;object-fit:cover;">
                        <span class="news-author">${autorNombre}</span>
                    </div>
...
function cargarNoticiasSlider(noticias) {
    ...
    // Esconder slider de noticias si no hay noticias.
    const hayNoticias = sliderNoticiasGlobal.length !== 0
    sliderBlock.style.display = hayNoticias ? "block" : "none" 
    ...
    sliderNoticiasGlobal.forEach((n, i) => {
        ...
        const tieneImagen = noticiaImg && String(noticiaImg).trim() !== '';
        const img = document.createElement('img');
        img.src = tieneImagen ? noticiaImg : '../img/logo-tecnica.png';
        img.alt = n.titulo || '';
