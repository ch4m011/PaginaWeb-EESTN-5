...
(function () {
	...
	async function init() {
		bindUI();
		currentAdminId = (await fetchSessionUser()).id || 0;
	...
	function renderUsersTable() {
      ...
      <tr>
        ...
        <td>${escapeHtml(user.coemailrreo)}</td>
        <td class="td-rol">
    ...
    `).join('');
		...
		deleteBtns.forEach(b => {
			b.addEventListener('click', async function () {
				...
				if (!(await eestConfirm(`¿Eliminar al usuario "${userNombre}"? Esta acción no se puede deshacer.`, { titulo: 'Eliminar usuario', textoConfirmar: 'Eliminar' }))) return;
				try {
	...
	async function sendPatch(user, patch) {
		try {
			const endpoint = "../php/usuarios.php?id=" + encodeURIComponent(user.id);
	...
	async function renderNoticias() {
		...
		for (const n of filtradas) {
			try {
				...
				const safeHtml = n.contenido || '';
				const extracto = textFromHtml(safeHtml, 120);

				card.innerHTML = `
        			<div style="width:100%;height:180px;overflow:hidden;">
        			  <img src="../php/imagen_noticia.php?id=${n.id}" alt="${escapeHtml(n.titulo)}" style="width:100%;height:100%;object-fit:cover;" onerror="this.onerror=null;this.src='../img/logo-tecnica.png';this.style.objectFit='contain';this.style.background='#fff';this.style.padding='20px';">
        			</div>
        			<div style="padding:12px;display:flex;flex-direction:column;gap:8px;">
        			  <h3 style="margin:0;color:#0b3b66;">${escapeHtml(n.titulo)}</h3>
        			  <small style="color:#666;">Por ${escapeHtml(noticia_usuario.nombre)} • ${escapeHtml(n.fecha)}</small>
				...
				card.querySelector(".btn-delete").addEventListener("click", async () => {
					const ok = await eestConfirm(`¿Eliminar la noticia "${n.titulo}"?`, { titulo: 'Eliminar noticia', textoConfirmar: 'Eliminar' });
					if (ok) eliminarNoticia(n.id);
				});
	...
	function viewNoticia(noticia) {
		...
		window.location.href = `../html/ver_noticia.html?id=${encodeURIComponent(noticia.id)}`;
	}
	...
	function editarNoticiaDesdeAdmin(id) {
		...
		window.location.href = "../html/panel_escritor.html";
	}
	...
	async function fetchSessionUser() {
		try {
			...
			return { id: parseInt(json.id || 0, 10) || 0 };
		} catch (e) {
	...
	// Usa el mismo editor visual que el panel del escritor.
	function initFormNoticiaAdmin() {
		...
		// Barra de herramientas igual a la del escritor.
		document.querySelectorAll('#formNoticia .editor-toolbar button[data-cmd]').forEach(btn => {
		...
		form.addEventListener('submit', async (ev) => {
			...
			try {
				...
				const contenidoTexto = editor?.textContent?.trim() || '';

				if (!titulo) throw new Error('El título es obligatorio');
				if (!contenidoTexto) throw new Error('El contenido es obligatorio');
				...
				let tipoSeleccion = document.querySelector('input[name="tipoNoticia"]:checked')?.value || 'secundaria';

				const payload = {
					titulo,
					contenido: contenidoHTML,
					resumen: contenidoTexto.substring(0, 150),
				...
				if (res.ok && (j.success || j.id)) {
					...
					const secundaria = document.querySelector('input[name="tipoNoticia"][value="secundaria"]');
					if (secundaria) secundaria.checked = true;
					fetchNoticias();
				} else {
					throw new Error(j.error || j.message || 'Error al guardar noticia');
				}
			} catch (err) {
	...
	async function fetchEventosGlobales() {
		try {
			const res = await fetch("../php/api_calendario.php?action=obtener&orden=ASC", { cache: "no-store" });
	...
	function renderEventosGlobales() {
        ...
        🕒 ${escapeHtml(ev.hora_inicio || ev.hora_inicio || "")} - ${escapeHtml(ev.hora_fin || ev.hora_fin || "")}<br>
      ...
      `;
			card.querySelector(".btn-delete").addEventListener("click", async () => {
				const ok = await eestConfirm(`¿Eliminar el evento "${ev.titulo}"?`, { titulo: 'Eliminar evento', textoConfirmar: 'Eliminar' });
				if (ok) eliminarEventoAdmin(ev.id);
			});
			cont.appendChild(card);
		});
	}
	...
	async function eliminarEventoAdmin(id) {
		try {
			const res = await fetch("../php/api_calendario.php?action=eliminar", {
				method: "DELETE",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({ id })
			});
		...
		form.addEventListener("submit", async (ev) => {
			...
			try {
				const datosEvento = {
					...
					hora_inicio: $("#hora_inicioAdmin")?.value || "",
					hora_fin: $("#hora_finAdmin")?.value || ""
				};

				if (!datosEvento.fecha || !datosEvento.titulo || !datosEvento.tipo || !datosEvento.hora_inicio || !datosEvento.hora_fin) {
					throw new Error("Completá todos los campos requeridos.");
				}

				const res = await fetch("../php/api_calendario.php?action=crear", {
					method: "POST",
					headers: { "Content-Type": "application/json" },
	...
	window.cambiarRol = async function (usuarioId, nuevoRolOrElement) {
		try {
			...
			if (!(await eestConfirm(`¿Confirmar cambio de rol a "${nuevoRol}" para el usuario ${usuarioId}?`, { titulo: 'Cambiar rol', textoConfirmar: 'Confirmar', peligroso: false }))) return;
