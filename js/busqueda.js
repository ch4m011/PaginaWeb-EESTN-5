...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    resultados.forEach(n => {
      ...
      div.innerHTML = `
        ${n.imagen ? `<img src="../php/img/${n.imagen.split('/').pop()}" alt="${n.titulo}">` : ""}
        <h3>${n.titulo}</h3>
  ...
  function mostrarMensaje(texto, incluirBoton = false) {
    cont.innerHTML = `
      <div class="mensaje">
        ...
        ${incluirBoton ? `<button class="btn-volver" onclick="window.location.href='../html/index.html'">Volver al inicio</button>` : ""}
      </div>
    `;
  }
});
