...
//   - get_user.php                  -> GET,  campos planos (ok, nombre, email...)
//   - perfil/actualizar_datos.php        -> POST JSON      {nombre}
//   - cambiar_contrasena.php       -> POST FormData  (password_actual, password_nueva) — NO admite JSON
//   - perfil/actualizar_imagen.php -> POST FormData con campo "avatar"
...
// Notificación flotante arriba a la derecha, igual que en panel_admin.
...
function actualizarUI() {
    ...
    if (inputCorreo) inputCorreo.value = usuario.email || '';
...
function configFormImagen() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            const resp = await fetch('../php/perfil/actualizar_imagen.php', {
                method: 'POST',
            ...
            if (data.ok) {
                ...
                if (imgAvatar) {
                    imgAvatar.src = data.imagen + '?t=' + Date.now();
                }
...
function configFormPerfil() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        setBotonCargando(form, true);
        try {
            const resp = await fetch('../php/perfil/actualizar_datos.php', {
                method: 'POST',
                credentials: 'same-origin',
...
function configFormEliminar() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        if (!(await eestConfirm('¿Seguro que deseas eliminar tu cuenta? Esta acción es irreversible.', { titulo: 'Eliminar cuenta', textoConfirmar: 'Eliminar cuenta' }))) {
            return;
        }
