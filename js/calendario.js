...
document.addEventListener("DOMContentLoaded", () => {
  ...
  formEvento.addEventListener("submit", async (e) => {
    ...
    if (!fecha || !titulo || !tipo || !hora_inicio || !hora_fin) {
    ...
    if (eventoEditando) {
      ...
      await actualizarEventoBD(eventoEditando, {
        fecha, titulo, tipo, descripcion, hora_inicio, hora_fin
      });
      eventoEditando = null;
    } else {
  ...
  async function crearEventoBD(datosEvento) {
    try {
      const res = await fetch("../datphpe/api_calendario.php?action=crear", {
        method: "POST",
  ...
  function editarEvento(id) {
    ...
    document.getElementById("hora_fin").value = ev.hora_fin;
  ...
  async function cargarEventosJSON(mine = false) {
    try {
      const url = `../php/api_calendario.php?action=obtener&orden=ASC${mine ? '&mine=1' : ''}`;
