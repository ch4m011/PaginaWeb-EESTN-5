document.addEventListener('DOMContentLoaded', () => {
  ...
  async function cargarEventosGlobales() {
    try {
      const res = await fetch('../php/api_calendario.php?action=obtener&orden=ASC');
  ...
  function renderLista(eventos) {
    ...
    eventos.forEach(ev => {
      ...
      card.innerHTML = `
        ...
        📅 ${ev.fecha} • 🕒 ${ev.hora_inicio} - ${ev.hora_fin}<br>
  ...
  async function eliminarEvento(id) {
    if (!(await eestConfirm('¿Eliminar este evento? Esta acción no se puede deshacer.', { titulo: 'Eliminar evento', textoConfirmar: 'Eliminar' }))) return;
    try {
  ...
  async function editarEvento(ev) {
    ...
    const nuevahora_inicio = prompt('Hora inicio (HH:MM):', ev.hora_inicio) || ev.hora_inicio;
    const nuevahora_fin = prompt('Hora fin (HH:MM):', ev.hora_fin) || ev.hora_fin;
    const nuevaDesc = prompt('Descripción:', ev.descripcion || '') || ev.descripcion;

    try {
      const res = await fetch('../php/api_calendario.php?action=actualizar', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          id: ev.id,
          ...
          hora_inicio: nuevahora_inicio,
          hora_fin: nuevahora_fin
        })
      });
      const r = await res.json();
      if (r.success) {
