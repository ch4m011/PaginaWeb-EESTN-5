...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const formatearHora = (hora_inicio, hora_fin) => {
    const inicio = String(hora_inicio || '').slice(0, 5);
    const fin = String(hora_fin || '').slice(0, 5);
  ...
  async function cargarEventos() {
    ...
    try {
      // index.html está dentro de /html/, por eso la API se referencia con ../php/.
      const respuesta = await fetch('../php/api_calendario.php?action=obtener&orden=ASC', {
        cache: 'no-store'
      });
  ...
  function renderizarEventos(eventos) {
    ...
    lista.innerHTML = eventos.map((evento) => {
      ...
      const hora = escapar(formatearHora(evento.hora_inicio, evento.hora_fin));
