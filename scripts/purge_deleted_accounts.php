...
require_once __DIR__ . '/../php/config.php'; // $mysqli

$grace_days = 7; // días de gracia antes de borrar
$sql = "SELECT id, imagen_perfil FROM usuarios WHERE deleted_at IS NOT NULL AND deleted_at < (NOW() - INTERVAL ? DAY)";
...
while ($row = $result->fetch_assoc()) {
    ...
    if (!empty($row['imagen_perfil'])) {
        $path = __DIR__ . '/../uploads/perfiles/' . basename($row['imagen_perfil']);
